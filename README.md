...
Introduction
ENHANCED ENCODER-DECODER  PROGRAM is a GUI utility that encodes alpha numeric characters into a code that requires the decoder to restore it. There is a  choice of  using a Shift Cipher  code or   using Morse code.  This program has AI  built into  it to guide  the user in using  this successfully. One example is when you utilize the Morse code  function the AI assistant  will alert you  if  you use lower case characters to encode. The output  will not be affected but the program is  designed for upper case Morse.
